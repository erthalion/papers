...
\begin{frame}[fragile]{}
    ...
    \begin{center}
        \begin{itemize}[label={\MVRightarrow}]
            \item Why sound processing?
            \item Sound Exchange
            \item Recording and formats
            \item Cleaning up
            \item Merging layers
            \item Adding effects
        \end{itemize}
...
\end{frame}
\note{
    Volume adjustment: The parameter  to  the  vol  effect  which would  make
    the audio as loud as possible without clipping.
}
...
\begin{frame}[fragile]{}
    ...
    sox audio.mp3 agc.mp3 compand \
        ...
        6:      \ # transfer function, with 6dB soft knee
             ...
             0, -40,     \    # do nothing below -40dB
            -40,-35,     \    # compand -40dB to -35dB
            -20,  0, -20 \    # everything else to -20dB
            -10 -60 1
        \end{minted}
    \end{center}
\end{frame}
\note{
    Automatic Gain Control (AGC) is an audio processing technique that is
    designed to smooth out volume differences between different parts of an
    audio.

    Compand - compress or expand the dynamic range of audio.
    If the list is preceded by a soft-knee-dB value, then the points at where
    adjacent line segments on the transfer function meet will be rounded by the
    amount given.

    In the example above very soft sounds (below -80dB) will be left untouched.
}
...
\begin{frame}[fragile]{}
    ...
    \begin{center}
        ...
        \includegraphics[width=0.6\textwidth]{agc.png}
        \captionof{figure}{SoX compand (compress and expand) effect}
...
\begin{frame}[fragile]{}
    ...
    \begin{center}
        ...
        \captionof{figure}{This PCM waveform is clipped between the red lines}
...
\end{frame}
\note{
    A high-pass filter (HPF) attenuates content below a cutoff frequency,
    allowing higher frequencies to pass through the filter. A low-pass filter
    (LPF) attenuates content above a cutoff frequency, allowing lower
...
\begin{frame}[fragile]{}
    ...
    \begin{center}
        ...
        \captionof{figure}{SoX sinc (band-pass filter) effect}
...
\begin{frame}[fragile]{}
    ...
    \begin{center}
        ...
        \captionof{figure}{Dithering in image processing.}
